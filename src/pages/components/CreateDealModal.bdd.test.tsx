...
vi.mock('@so360/shell-context', () => ({
  ...
  usePeople: (...a: any[]) => mockUsePeople(...a),
...
beforeEach(() => {
  ...
  mockUsePeople.mockReturnValue({
    people: [
      { id: 'p2', full_name: 'Zara Patel', job_title: 'Sales Executive', status: 'active' },
      { id: 'p1', full_name: 'Alan Rep', job_title: 'Growth Manager', status: 'active' },
    ],
  });
});
...
describe('CreateDealModal', () => {
  ...
  describe('Given the Sales Rep dropdown needs People Connect data', () => {
    it('When rendered / Then usePeople is called with the current org, active-only filter, and enabled true', () => {
      render(<CreateDealModal {...defaultProps} />);
    ...
    it('When People Connect has no active employees yet / Then Sales Rep dropdown shows no rep options', () => {
      mockUsePeople.mockReturnValue({ people: [] });
      render(<CreateDealModal {...defaultProps} />);
      expect(screen.queryByText('Alan Rep (Growth Manager)')).not.toBeInTheDocument();
      expect(screen.queryByText('Zara Patel (Sales Executive)')).not.toBeInTheDocument();
    });
