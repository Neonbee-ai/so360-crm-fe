...
const FORWARD_STATES = ['new', 'qualified', 'proposal', 'negotiation', 'won'] as const;
...
export const DealLifecycleStepper: React.FC<DealLifecycleStepperProps> = ({ currentState }) => {
    ...
    return (
        <div className="flex items-center gap-0">
            {FORWARD_STATES.map((state, index) => {
                ...
                const isTerminalWon = isLast && (isCompleted || isCurrent);
                    ...
                    <React.Fragment key={state}>
                        <div className="flex flex-col items-center">
                            <div className={`flex items-center justify-center w-8 h-8 rounded-full transition-all ${
                                ...
                                isCurrent ? (state === 'won' ? 'bg-emerald-500/20 border-2 border-emerald-500 text-emerald-400' : 'bg-teal-500/20 border-2 border-teal-500 text-teal-400') :
                                'bg-slate-800 border border-slate-600 text-slate-600'
                            }`}>
                                {isCompleted ? <CheckCircle className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                            </div>
                            <span className={`text-xs mt-2 text-center w-16 ${
                                isCompleted || isCurrent ? (state === 'won' ? 'text-emerald-400' : 'text-teal-400') : 'text-slate-600'
                            }`}>
                                {STATE_LABELS[state]}
                            </span>
                        </div>
